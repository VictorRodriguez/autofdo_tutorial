...
def generate_perf_data(command):
    logger.info("Generating perf file")
    p = subprocess.Popen(os.path.join(os.path.dirname(os.path.realpath(__file__)), "generate_perf_data.sh -c=\""+command+"\""), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = p.communicate()
    logger.info(out)
    if p.returncode:
        logger.error(err)
        logger.error("Perf file generation error")
        sys.exit(1)
    perf_file = command.split()[0]
    if "/" in perf_file:
        perf_file = perf_file.split("/")[-1]
    if "." in perf_file:
        perf_file = perf_file.split(".")[0]
    if not os.path.exists(perf_file) or not os.path.isfile(perf_file):
        logger.error("Perf data file wasn't created")
        sys.exit(1)
    return perf_file
...
if __name__ == "__main__":	
    parser = argparse.ArgumentParser(prog='./gcov_generator.py',
                                     usage='%(prog)s [options]')
    parser.add_argument('-c', '--command', dest='command', action='store', help='Command to execute benchmark'
    ...
    perf_file = generate_perf_data(args.command)
